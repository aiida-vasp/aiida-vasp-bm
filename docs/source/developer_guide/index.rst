...
``aiida-vasp-bm`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-vasp-bm`` repository (preferably using ``aiida-vasp-bm`` as the project name)

The documentation is now available at `aiida-vasp-bm.readthedocs.io <http://aiida-vasp-bm.readthedocs.io/>`_.
